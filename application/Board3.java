...
public class Board3 extends JPanel implements ActionListener{
	...
	private Tetrominos[] board3;
	public Tetris parent;
	public Board3(Tetris parent) {
		this.parent = parent;
		setFocusable(true);
		curPiece = new Shape(); 		
		...
		board3 = new Tetrominos[BOARD_WIDTH * BOARD_HEIGHT];
		clearBoard();
	...
	private void pieceDropped() {
		for(int i = 0; i < 4; i++) {
			...
			board3[y * BOARD_WIDTH + x] = curPiece.getShape();
		}
	...
	public void newPiece() {
		...
		if(!tryMove(curPiece, curX, curY - 1)) {
			...
			GameOver3 Over3 = new GameOver3(parent);
			add(Over3);
			statusBar.setText("0");
		...
		for(int i = BOARD_HEIGHT - 1; i >= 0; i--) {
			...
			if(LineFull) {
				...
				for(int k = i; k < BOARD_HEIGHT - 1; k++) {
					for(int j = 0; j < BOARD_WIDTH; j++) {
						board3[k * BOARD_WIDTH + j] = shapeAt(j, k + 1);
					}
				}
			}
